...
class Ensh extends TagLib {
    protected $tags = array(
        ...
        'tlist' => array('attr' => 'db,order,limit,where,fields,id,key','close' => 1),
    );
    /**
     * 这是一个闭合标签的简单演示
     */
    ...
    public function tagTlist($attr,$content) {
        ...
        $where = isset($attr['where'])?$attr['where']:''; //查询条件
        $field = isset($attr['field'])?$attr['field']:'*'; //查询字段
        $id = isset($attr['id'])?$attr['id']:''; //查询字段
        $key = isset($attr['key'])?$attr['key']:'k';
        $str = '<?php ';
        $str.='$result = db("'.$db.'")->where("'.$where.'")->field('.$field.')->limit('.$limit.')->order("'.$order.'")->select();';
